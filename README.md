...
- **Docker** (optional)
  - Easiest cross-platform setup for PostgreSQL.
...
### Database Creation
```bash
psql -U postgres -c "CREATE DATABASE starforge_dw;"
```

### Docker Quickstart (Optional)
```bash
docker run --name starforge_dw \
  -e POSTGRES_PASSWORD=postgres \
  -e POSTGRES_DB=starforge_dw \
  -p 5432:5432 \
  -d postgres:16

# Wait a few seconds for Postgres to start, then run from the repo root:
psql -h localhost -U postgres -d starforge_dw -f sql/all_scripts_combined.sql
```
...
```bash
psql -U postgres -d starforge_dw -f sql/01_create_staging_tables.sql
psql -U postgres -d starforge_dw -f sql/02_load_staging_data.sql
```

### 4. Create Dimension & Fact Tables
```bash
psql -U postgres -d starforge_dw -f sql/03_create_dim_tables.sql
psql -U postgres -d starforge_dw -f sql/04_create_fact_tables.sql
```

### 5. ETL for Dimensions & Facts
```bash
psql -U postgres -d starforge_dw -f sql/05_etl_dim_facts.sql
```

### 6. Apply SCD Upserts
```bash
psql -U postgres -d starforge_dw -f sql/06_scd_upserts.sql
```

### 7. Add Constraints & Indexes
```bash
psql -U postgres -d starforge_dw -f sql/07_add_constraints_indexes.sql
```

### 8. Run Analytics Queries
```bash
psql -U postgres -d starforge_dw -f sql/ANALYTICS.sql
```

*Tip: Run these commands from the repo root so relative CSV paths resolve correctly. You can also use all_scripts_combined.sql to execute everything in one shot.*
